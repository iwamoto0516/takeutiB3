...
public class Takahashi3 extends Player {
    private Random random = new Random();
    // 探索の深さ：3手
    private final int MAX_DEPTH = 3; 
    //継承
    public Takahashi3(int color) {
        super(color);
    }
        ...
        if (walls > 0) {
            ...
            for (int r = minR; r <= maxR; r++) {
                for (int c = minC; c <= maxC; c++) {
                    ...
                    if (wp.placeHorizontalWall(cloneH, r, c)) {
                        Move m = new Move();
                        m.isPawnMove = false;
                        m.isHorizontal = true;
                        m.r = r;
                    ...
                    if (wp.placeVerticalWall(cloneV, r, c)) {
                        Move m = new Move();
                        m.isPawnMove = false;
    ...
    private boolean moveToShortestPath(Board board, PawnMove pawnMove, int myRow, int myCol, int goalRow) {
        ...
        //自分の周囲1マスのみ探索
        int startR = Math.max(0, myRow - 1);
        int endR = Math.min(8, myRow + 1);
        int startC = Math.max(0, myCol - 1);
        int endC = Math.min(8, myCol + 1);
